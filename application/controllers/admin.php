...
class Admin extends CI_Controller {
		...
		if ($this->input->post('submit')) 
		{
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($this->admin_model->cek_user() == TRUE) {
					...
					// redirect(base_url('admin/home'));
					$data['notifSuccess'] = 'Login Berhasil';
					$this->load->view('login2', $data);
					header("Refresh:.2;url=".base_url()."admin/home");
				} else {
	...
	{
		if ($this->session->userdata('logged_in') == TRUE && ($this->session->userdata('level') == 'Admin')) {
			$data['main_view'] = 'waiting';
			$data['waiting'] = $this->admin_model->get_waiting();
			$data['jenis'] = $this->admin_model->get_jenis();
			$data['akun'] = $this->admin_model->get_akun();
			$this->load->view('template', $data);
		} else if ($this->session->userdata('logged_in') && !($this->session->userdata('level') == 'Admin')) {
	...
	public function approve()
	{
		if ($this->session->userdata('logged_in') == TRUE && ($this->session->userdata('level') == 'Admin')) {
			...
			$data = $this->db->query('SELECT * FROM tb_informasi WHERE id_informasi = '.$id_informasi.'');

			$approve = $this->db->query('UPDATE tb_informasi SET accept = "approve" WHERE id_informasi = '.$id_informasi.'');
			if ($approve) {
				$push = $this->admin_model->push_notif("/topics/testing", $data->nama_event, $data->deskripsi, "");
				redirect('admin/waiting','refresh');
			} else{
				redirect('admin/waiting','refresh');
			}
